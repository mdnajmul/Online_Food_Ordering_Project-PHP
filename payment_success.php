<?php
   ...
   include('smtp/class.phpmailer.php');
    ...
    if($code == 200 && !( curl_errno($handle)))
    {
        ...
        mysqli_query($con,"UPDATE order_master SET payment_status='success',online_payment_status='$status', pay_id='$val_id',payment_method='$card_type' WHERE txnid='$tran_id'");
        
        //hold order id from order table for using invoice sent to user email
        $order_details=mysqli_fetch_assoc(mysqli_query($con,"SELECT id FROM order_master WHERE txnid='$tran_id'"));
        
        
        $email=$_SESSION['FOOD_ORDER_ADDRESS_EMAIL'];
        $order_id=$order_details['id'];
        
        //call 'orderInvoiceSendEmail()' function for html data Invoice which sent to user email when order place 
        $html=orderInvoiceSendEmail($order_id);

        //call 'send_email()' function for send email
        send_email($email,$html,'Order Placed Invoice');
        ...
        redirect(SITE_PATH.'payment_fail');
